...
python3 -m pip install ${PIP_FLAGS} --no-cache-dir setuptools wheel scikit-build-core
echo "[*] Installing python packages (forcing native compiler)"
export CC=gcc
export CXX=g++

python3 -m pip install ${PIP_FLAGS} --no-cache-dir \
  'langchain>=0.1.0,<0.3' \
  'langchain-community>=0.0.20'
python3 -m pip install ${PIP_FLAGS} --no-cache-dir --no-build-isolation \
  'llama-cpp-python>=0.2.0'
python3 -m pip install ${PIP_FLAGS} --no-cache-dir packaging
